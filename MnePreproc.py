import mne, os, glob, time, pickle
import numpy as np
...
class MnePreproc:
    ...
    def __init__(self,subject,experiment,root,trigger_scheme,condition_format=None):
        ...
        self.processing_info = 'preprocessing date: %s' % time.strftime("%d %b %Y")
    ...
    def load_raw(self, filename=None):
        ...
        self.add_preprocessing_notes("%s total events found.\n%s events found with triggers in trigger scheme." %(str(len(event_check)), str(len(exp_event_check))))
    ...
    def add_info_to_events(self):
        ...
        ds['condition'] = Factor([self.inverted_triggers[t] for t in ds['trigger']])

        try:
            for i, f in enumerate(self.factors):
                ds[f] = Factor(self.inverted_triggers_levels[t][i] for t in ds['trigger'])
        except AttributeError:
            pass
    ...
    def prepare_raw_and_epoch(self, tmin, tmax, baseline=(None, 0), filter='0-40'):
    ...
    def gui_blink_reject(self,epochs,eye_channels='AD'):
        ...
        n_before, n_after = len(epochs), len(epochs_clean)
    ...
    def apply_blink_rejections(self):
        ...
        if len(rej_files) > 1:
            ...
            if op.splitext(rej_files[0])[1] == '.pickled':
                blink = load.unpickle(rej_files[0])
            elif op.splitext(rej_files[0])[1] == '.txt':
                blink = load.tsv(rej_files[0])

        idx = blink['accept'].x
        self.epochs_clean = self.epochs[idx]
        self.ds_events['survived_rej'] = Factor(idx)

        print "Rejecting %s epochs as determined by the rejection file %s" %(str(len(self.epochs)-len(self.epochs_clean)), op.basename(rej_files[0]))

        return self.epochs_clean


    def make_evoked_per_condition(self, factorial=False):
        ...
        for i, key in enumerate(self.trigger_scheme):
            if key in self.ds_events['condition']:
